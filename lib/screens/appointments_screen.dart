...
class _AppointmentsScreenState extends State<AppointmentsScreen> {
  ...
  Future<String?> _getPatientPhoneNumber(String patientName) async {
    final records = await _databaseService.getAllPregnancyRecords();
    final patientRecord = records.firstWhere(
      (record) => record.motherName == patientName,
      //  orElse: () => null,
      orElse: () => throw Exception('Patient not found'),
    );
    return patientRecord.phone;
  }

  Future<void> _sendAppointmentSMS(
    String phoneNumber,
    DateTime dateTime,
    String purpose,
    String patientName,
    ...
    try {
      final formattedDate = DateFormat('EEEE, MMMM d, y').format(dateTime);
...
Please arrive 30 minutes early. Contact us if you need to reschedule.

~CHPS MamaCare,
Safe Pregnancies, Healthy Community🤰❤.
''';
      ...
      if (await canLaunchUrl(smsUrl)) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('SMS app opened with appointment details!'),
            backgroundColor: Colors.green,
          ),
        );
      } else {
  ...
  void _scheduleAppointment() async {
    ...
    try {
      ...
      // Get patient phone number and send SMS
      final phoneNumber = await _getPatientPhoneNumber(_selectedPatient!);
      if (phoneNumber != null && phoneNumber.isNotEmpty) {
        await _sendAppointmentSMS(
          phoneNumber,
          dateTime,
          _selectedPurpose!,
          _selectedPatient!,
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Appointment scheduled but no phone number found for SMS',
            ),
            backgroundColor: Colors.orange,
          ),
        );
