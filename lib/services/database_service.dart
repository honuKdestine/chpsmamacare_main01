...
class DatabaseService {
  ...
  // Get pregnancy records that still have local (non-URL) file paths
  Future<List<PregnancyRecord>> getRecordsWithLocalFiles() async {
    final allRecords = await getAllPregnancyRecords();
    return allRecords.where((record) {
      final paths = record.testFilePaths ?? [];
      return paths.any((p) => !p.isUrl); // Uses your _UrlCheck extension above
    }).toList();
  }
}
